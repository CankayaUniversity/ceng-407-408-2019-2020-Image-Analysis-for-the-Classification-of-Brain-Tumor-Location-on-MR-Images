...
cursor = conn.cursor()
path = glob.glob("C:/Users/PC/Desktop/data1/*.jpg")

for imager in path:

    img = cv2.imread(imager)
    #gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    gray = cv2.imread(imager,0)
    a=pyodbc.Binary(gray)
    cursor.execute("insert into original(image,image_name,type,isTumor,isCancerous) values (?,?,?,?,?)", a,imager,'yes',0,0)
    conn.commit()
    ...
    cv2.waitKey(0)
    sql = "SELECT id FROM original WHERE image_name=  ?"
    cursor.execute(sql,imager)
    rows = cursor.fetchone()
    id=rows[0]
    b=pyodbc.Binary(result_image)
    cursor.execute("insert into segmented(oi_id, isSegmented,seg_image) values (?,?,?)",id,1,a)
    conn.commit()